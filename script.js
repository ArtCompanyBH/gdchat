...
// Opcional (existe no seu HTML): status do bot
...
let isSending = false; // evita spam/duplo clique enquanto espera resposta
...
function smartScroll(forceScroll = false) {
  ...
  const isNearBottom =
    chat.scrollHeight - chat.scrollTop - chat.clientHeight < SCROLL_BOTTOM_THRESHOLD;
...
// Botao copiar (somente para mensagens do bot)
function createCopyButton(botText) {
  ...
  // Estilo inline leve para nao depender de CSS novo
...
function addMessage(role, content) {
  ...
  const displayText = rolePrefix + formatted;

  // Preserva quebras de linha (sem depender do CSS)
  div.style.whiteSpace = "pre-wrap";
  div.textContent = displayText;

  appendTimestamp(div);

  // Botao copiar (apenas bot)
  if (role === "bot") {
    // copia somente o texto da resposta (sem o prefixo)
    const copyBtn = createCopyButton(formatted);
...
function renderHistory() {
  ...
  // Renderiza historico sem regravar no cache
...
function showHelp() {
  ...
  addSystemMessage("/status - Mostra infos rápidas do app");
}

function showStatus() {
  const count = chatHistory.length;
  const lastSend = lastMessageTime ? new Date(lastMessageTime).toLocaleString() : "nunca";
  addSystemMessage(`ℹ️ Status: ${count} mensagens no cache | Último envio: ${lastSend}`);
}
...
// ========== API Send (apenas mensagem atual) ==========
async function sendMessage(message) {
  ...
  // Evita duplo envio enquanto aguarda resposta
  if (isSending) {
  ...
  try {
    ...
    const botResponse =
      ...
      "⚠️ Resposta inesperada (sem texto).";
...
sendButton.addEventListener("click", () => {
  ...
  // limpa agora, mas restaura se rate-limit/busy (melhor UX)
  textInput.value = "";
